...
class DC_STN(object):
  ...
  def __repr__(self):
    ...
    for (i,j) in edge_keys:
      ...
      if (for_edge.fake or rev_edge.fake) and all(map(lambda e: e==None,spec_edges)):
        continue
  ...
  def no_case_reductions(self,debug):
    ...
    # at a special node
    for first_edge in self.normal_edges.values():
  ...
  def upper_case_reductions(self,debug):
    ...
    # special nodes
    for uc_edge in self.upper_case_edges.values():
  ...
  def cross_case_reductions(self,debug):
    num_reductions = 0
    for lc_edge in self.lower_case_edges.values():
