...
import traceback
import argparse
sys.path.append(os.getenv("HOME")+"/pybin/PuLP-1.6.0/src")
from pulp import *
...
def setUpLP(STN, decouple):
  ...
  prob = LpProblem('PSTN Robust Execution LP',LpMaximize)
  ...
  for i in STN.verts:
    bounds[(i,'+')] = LpVariable('t_%d_hi'%i, lowBound = None,
                                             upBound = STN.getEdgeWeight(0,i))

    bounds[(i,'-')] = LpVariable('t_%d_lo'%i, lowBound = -STN.getEdgeWeight(i,0),
                                             upBound = None)
    addConstraint( bounds[(i,'+')] >= bounds[(i,'-')] , prob)

  for i,j in STN.edges:
    if (i,j) in STN.contingentEdges:
      deltas[(i,j)] = LpVariable('delta_%d_%d'%(i,j), lowBound = 0, upBound = None)
      deltas[(j,i)] = LpVariable('delta_%d_%d'%(j,i), lowBound = 0, upBound = None)

    else:
      # ignore edges from z. these edges are implicitly handled with the bounds
      # on the LP variables
      if i != 0 and not decouple:
        addConstraint(bounds[(j,'+')]-bounds[(i,'-')] <= STN.getEdgeWeight(i,j),prob)
        addConstraint(bounds[(i,'+')]-bounds[(j,'-')] <= STN.getEdgeWeight(j,i),prob)
    ...
    if upper - lower <= 1:
      if result != None:
        ...
        for i,sign in LPbounds:
          if sign == '+':
            inputSTN.updateEdge(0,i,ceil(bounds[(i,'+')].varValue))
          else:
            inputSTN.updateEdge(i,0,ceil(-bounds[(i,'-')].varValue))
...
def srea_LP(inputSTN,
  ...
  if type(invCDF_map) != dict:
    raise TypeError("invCDF_map is not of type dict.")
