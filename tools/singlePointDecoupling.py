...
## \file singlePointDecoupling.py
#
#  \brief Solves the naive linear program for decoupling a schedule at a specific inter-agent constraint
#
...
def main():
  ...
  for edge in originalEdges:
    ...
    # Desired inter-agent constraint
    if doubleSTN[i]['agent'] != doubleSTN[j]['agent']:
      if (node1 == i and node2 == j) or (node1 == j and node2 == i):
